...
namespace NzbDrone.Core
{
    public class CentralDispatch
    {
        ...
        private void InitDatabase()
        {
            ...
            ContainerBuilder.Register(c => c.Resolve<Connection>().GetMainPetaPocoDb(databaseType))
                            .As<IDatabase>();

            ContainerBuilder.Register(c => c.Resolve<Connection>().GetLogPetaPocoDb(databaseType, false))
                            .SingleInstance()
                            .Named<IDatabase>("DatabaseTarget");

            ContainerBuilder.Register(c => c.Resolve<Connection>().GetLogPetaPocoDb(databaseType))
